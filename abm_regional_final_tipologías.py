# -*- coding: utf-8 -*-
"""abm_regional_final_tipologías.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1eRY7qvWOUFwW2bJtxJKXlT3A_L0BvqcT
...
class EvaluacionDinamica:
    ...
    def evaluar_evolucion_dinamica(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not distribucion_actualizada.empty:
            ...
            # Asegurar que siempre retorne el DataFrame completo
            print(f"Distribución calculada para {len(distribucion_actualizada)} pasos de tiempo")

        else:
            # Si no hay datos, agregar columna de distribución por defecto
            df_resultado['distribucion_inicial'] = df_resultado['paso_tiempo'].map(
                lambda x: True if x == 0 else False
            )
    ...
    def crear_tipologia_regional(n_region: int, n_pasos: int, distribucion_inicial: Dict[TipoRegionEconomica, float] = None) -> pd.DataFrame:
        ...
        # Usar distribución proporcionada o empírica por defecto
...
class ModeloEconomicoRegional:
    ...
    def ejecutar_simulacion(self, num_pasos: int = 60) -> pd.DataFrame:
        ...
        # Aplicar evaluación dinámica para distribución actualizada
        print("Aplicando evaluación dinámica para distribución actualizada...")
        df_resultados = EvaluacionDinamica.evaluar_evolucion_dinamica(df_resultados)

        print(f"Simulación completada. {len(df_resultados)} registros generados.")
        print("Distribución dinámica aplicada - las proporciones 75-25 se actualizan por paso de tiempo")
...
def calcular_correlacion_segura(df):
    ...
    try:
        ...
        # CORREGIDO: Validaciones robustas adicionales
        # Verificar que no hay columnas con todos los valores iguales
        for col in df_clean.columns:
            if df_clean[col].nunique() <= 1:
                df_clean = df_clean.drop(columns=[col])

        # Verificar que hay suficiente variabilidad
        if len(df_clean.columns) < 2 or len(df_clean) < 3:
            return pd.DataFrame()

        corr = df_clean.corr()

        # Verificar que la matriz de correlación es válida
            ...
            if i != j:
                ...
                if tipologia_a != tipologia_b:
                    ...
                    if datos_a_info['len'] == datos_b_info['len'] and datos_a_info['len'] > 3:
                        ...
                        # CORREGIDO: Validaciones robustas mejoradas para spillovers
                        if (var_cap_a > 0 and var_cap_b > 0 and var_innov_a > 0 and var_innov_b > 0 and
                            datos_a_info['len'] > 5 and datos_b_info['len'] > 5):  # Más puntos de datos

                            # Verificar que hay suficientes datos para correlación
                            min_datos = min(len(datos_a), len(datos_b))
                            if min_datos > 10:  # Al menos 10 puntos para correlaciones confiables
                                ...
                                # Validación robusta de correlaciones
                                if (not (np.isnan(corr_capacidad) or np.isnan(corr_innovacion)) and
                                    abs(corr_capacidad) <= 1 and abs(corr_innovacion) <= 1):

                                    # Calcular intensidad spillover con ponderación mejorada
                                    # Combina correlaciones directas y promedio ponderado
                                    intensidad_directa = abs(corr_capacidad) * 0.4 + abs(corr_innovacion) * 0.3
                                    intensidad_promedio = (abs(corr_capacidad) + abs(corr_innovacion)) / 2 * 0.3
                                    intensidad_spillover = intensidad_directa + intensidad_promedio

                                    # Agregar variación de capacidad como métrica adicional
                                    variacion_capacidad = abs(var_cap_a - var_cap_b) / max(var_cap_a, var_cap_b)
                                    ...
                                    spillovers_tecnologicos.append({
                                        ...
                                        'variacion_capacidad_relativa': variacion_capacidad,
                                        'n_datos_correlacion': min_datos
                                    })
