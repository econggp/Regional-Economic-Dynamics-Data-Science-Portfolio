# -*- coding: utf-8 -*-
"""abm_tipologías_final.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1rgIrQesHnr2Ta-fLycXdpk2b4WVBKckc
...
class EcuacionesCalibradas:
    ...
    def evolucion_capacidad_tecnologica(capacidad_actual: float, innovacion: float,
        ...
        dC/dt = α*I*(1-C) + β*ln(1+D) - γ*S*C + δ*(1-φ)
    ...
    def probabilidad_transicion_tipologia(capacidad_tecnologica: float, diversificacion: float,
        ...
        P(transición) = 1 / (1 + e^(-β₀ - β₁C - β₂D - β₃S - β₄I))
...
class ParametrosGlobales:
    ...
    """
      ...
      - Diferencia productividad: 2.3x entre tipos
    ...
    TAMAÑO_POOL_TRABAJADORES = 5700000  # PEA México 2024 57000000
    ...
    TIEMPO_RECUPERACION_CRISIS_MESES = 2  # Tiempo promedio recuperación
    ...
    def obtener_parametros_tipologicos() -> Dict[TipoRegionEconomica, ParametrosTipologicos]:
        ...
        return {
            TipoRegionEconomica.ATRAPADA: ParametrosTipologicos(
                ...
                capacidad_tecnologica_base=0.31,  # (baja, sectores tradicionales)
                innovaciones_base=0.02,  # Baja tasa de innovación
                tasa_innovacion_base=0.02,
                diversidad_base=0.38,  # Baja diversificación
                especializacion_base=0.72,  # Alta especialización
                brecha_calificacion_base=0.52,  # Moderada/alta brecha de calificación
                ...
                factor_intensidad_laboral=3.0 # Menor intensidad laboral calificada
            ),
            ...
            TipoRegionEconomica.NO_ATRAPADA: ParametrosTipologicos(
                ...
                capacidad_tecnologica_base= 0.89,  # Alta capacidad tecnológica
                innovaciones_base=0.08,  # Alta tasa de innovación
                tasa_innovacion_base=0.08,
                diversidad_base=0.71,  # Diversificación moderada (equilibrada con especialización)
                especializacion_base=0.40,  # Especialización moderada/intermedia
                brecha_calificacion_base=0.68,  # Alta brecha de calificación
...
class Region:
    ...
    def ejecutar_paso_simulacion(self, paso_tiempo: int) -> Dict:
        ...
        for fuerza_laboral in self.fuerzas_laborales.values():
            # Contar solo trabajadores con habilidad > umbral (empleo especializado)
            empleo_especializado += sum(
                1 for trabajador in fuerza_laboral.trabajadores
                if trabajador.habilidad > UMBRAL_ESPECIALIZACION
            )
...
def crear_visualizaciones_individuales(resultados: pd.DataFrame, save_path: str):
    ...
    if 'empleo_especializado' in resultados.columns:
        empleo_esp_limpio = resultados.dropna(subset=['empleo_especializado'])
        if not empleo_esp_limpio.empty:
            empleo_esp_tipologia = empleo_esp_limpio.groupby('tipo_region_economica_inicial')['empleo_especializado'].mean()

            # Verificar que hay datos válidos
            if not empleo_esp_tipologia.empty:
                ...
                for i, v in enumerate(empleo_esp_tipologia.values):
                    plt.text(i, v + max(empleo_esp_tipologia.values) * 0.02, f'{v:.1f}', ha='center', fontweight='bold')
            else:
            ...
            if 'capacidad_tecnologica' in resultados_proxy.columns and 'especializacion' in resultados_proxy.columns:
                ...
                if not atrapadas_tipologia.empty:
                    bars12 = plt.bar(atrapadas_tipologia.index, atrapadas_tipologia.values,
                                     color=[colores_tipologia[tipo] for tipo in atrapadas_tipologia.index],
                                     alpha=0.7, edgecolor='black')
                    plt.title('Regiones en Trampa de Innovación (%)\n(Criterio: Alta Especialización + Baja CapTech)', fontweight='bold', fontsize=14)
                    plt.ylabel('Porcentaje Atrapadas (%)', fontsize=12)
                    plt.xlabel('Tipología regional', fontsize=12)
                    for i, v in enumerate(atrapadas_tipologia.values):
                        plt.text(i, v + max(atrapadas_tipologia.values) * 0.02, f'{v:.1f}%', ha='center', fontweight='bold')
...
def crear_graficos_correlacion(resultados: pd.DataFrame, save_path: str):
    ...
    fig.suptitle('Matrices de correlación por categorías\nModelo ABM económico regional',
    ...
    if variables_corporativas_filt:
        ...
        if not corr_corporativa.empty:
            sns.heatmap(corr_corporativa, annot=True, cmap='RdBu_r', center=0,
                       ...
                       fmt='.2f', annot_kws={'size': 8})
            axes[1,0].set_title('Correlaciones corporativos\n(Empleo, Productividad, Innovación)',
                               fontweight='bold', fontsize=12)
            ...
            axes[1,0].set_title('Correlaciones corporativos', fontweight='bold')
    else:
    ...
    if variables_tecnologicas_filt:
        ...
        if not corr_tecnologica.empty:
            sns.heatmap(corr_tecnologica, annot=True, cmap='RdBu_r', center=0,
                       ...
                       fmt='.2f', annot_kws={'size': 8})
            axes[1,1].set_title('Correlaciones actualización tecnológica\n(Capacidad, Innovación, Actualización)',
                               fontweight='bold', fontsize=12)
            ...
            axes[1,1].set_title('Correlaciones actualización tecnológica', fontweight='bold')
    else:
    ...
    if len(variables_representativas) > 1:
        ...
        if not corr_cruzado.empty and len(corr_cruzado.columns) > 1:
            try:
                ...
                plt.title('Correlaciones cruzadas entre categorías\nMatriz de interdependencias principales',
                ...
                plt.text(0.5, 0.5, f'Error al generar correlaciones\ncruzadas: {str(e)[:50]}',
                        ha='center', va='center', fontsize=14, bbox=dict(boxstyle='round', facecolor='lightyellow'))
                plt.title('Correlaciones cruzadas entre categorías', fontsize=14, fontweight='bold')
                plt.tight_layout()
                plt.savefig(os.path.join(save_path, '20_correlaciones_cruzadas_categorias.png'),
                            dpi=300, bbox_inches='tight')
                plt.close()
        else:
            # Mensaje cuando no hay suficientes datos válidos
            plt.figure(figsize=(10, 8))
            mensaje = 'Datos insuficientes para\ncorrelaciones cruzadas'
            if len(variables_representativas) <= 1:
                mensaje = 'Solo una categoría disponible\npara correlaciones cruzadas'
            plt.text(0.5, 0.5, mensaje, ha='center', va='center', fontsize=16,
        ...
        # Mensaje cuando no hay suficientes categorías
        plt.figure(figsize=(10, 8))
        plt.text(0.5, 0.5, 'Categorías insuficientes\npara correlaciones cruzadas', ha='center', va='center',
...
if __name__ == "__main__":
    ...
    # Crear pool con los nuevos porcentajes: 65% servicios, 25% industria, 10% primario
    pool_trabajadores = DistribucionTrabajadores.crear_pool_trabajadores(tamaño_pool=5700000)

    print(f"Pool creado con {len(pool_trabajadores):,} trabajadores")
    print("Distribución aplicada:")
    print("• Sector Terciario (Servicios): 65%")
    print("• Sector Secundario (Industrial): 25%")
