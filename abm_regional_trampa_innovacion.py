# -*- coding: utf-8 -*-
"""abm_ti_regional_mx.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16cIkoXhWBqPwUkbooydsHgpp-1Qxr-fF
"""
...
def setup_matplotlib_safe():
    """Alias para setup_matplotlib_definitivo"""
    return setup_matplotlib_definitivo()
    ...
    def obtener_parametros_tipologicos() -> Dict[TipoRegionEconomica, ParametrosTipologicos]:
        ...
        return {
            ...
            TipoRegionEconomica.ATRAPADA: ParametrosTipologicos(
                ...
                # Parámetros Dinámicos (Críticos para la Trampa)
            ...
            TipoRegionEconomica.NO_ATRAPADA: ParametrosTipologicos(
                ...
                capacidad_tecnologica_base=0.70,
                innovaciones_base=0.08,
                tasa_innovacion_base=0.07,

                # Estructura: Ecosistema complejo y diversificado (Clusterización)
                diversidad_base=0.65,
                especializacion_base=0.50,          # Especialización inteligente (Smart Specialization)
...
class EcuacionesCalibradas:
    ...
    # FUENTE ÚNICA: los valores se leen directamente de ParametrosGlobales
    # para evitar redundancias y garantizar que un solo cambio propague a todo el modelo.
    ELASTICIDADES = {
        'capital':        ParametrosGlobales.ELASTICIDAD_CAPITAL,
        'labor':          ParametrosGlobales.ELASTICIDAD_LABOR,
        'capital_humano': ParametrosGlobales.ELASTICIDAD_CAPITAL_HUMANO,
        'innovacion':     ParametrosGlobales.ELASTICIDAD_INNOVACION,
        'inversion_id':   ParametrosGlobales.ELASTICIDAD_ID,
        'infraestructura':ParametrosGlobales.ELASTICIDAD_INFRAESTRUCTURA
    }
...
class EvaluacionDinamica:
    ...
    def evaluar_evolucion_dinamica(df: pd.DataFrame, columna_estado_previo: str = None) -> pd.DataFrame:
        ...
        for index, row in df_resultado.iterrows():
            ...
            ise_valor = EvaluacionDinamica.calcular_indice_salud_ecosistema(
            ...
            nuevo_estado = EvaluacionDinamica.determinar_estado_transicion(str(estado_previo), ise_valor)

            estados_dinamicos.append(nuevo_estado)
            ise_lista.append(ise_valor)
            # Propensión inversa al índice de salud
            propensiones.append(1.0 - ise_valor)

            # Actualizar previo para la siguiente iteración
            estado_previo = nuevo_estado

        df_resultado['estado_dinamico'] = estados_dinamicos
        df_resultado['indice_salud_ecosistema'] = ise_lista
...
class GestorInteracciones:
    # Matriz topológica espacial de México (Macro-Regiones)
    # Valores empíricos invertidos: Menor valor = Mayor cercanía geográfica y económica
    # 1.0 (Idénticas), 1.2-1.5 (Adyacentes/Alta Integración), 2.0-2.5 (Intermedias), 3.0+ (Lejanas/Aisladas)
    _DISTANCIAS = {
        # Desde NOROESTE
        ('Noroeste', 'Noroeste'): 1.0, ('Noroeste', 'Noreste'): 1.5,
        ('Noroeste', 'Occidente'): 1.8, ('Noroeste', 'Centronorte'): 2.0,
        ('Noroeste', 'Oriente'): 2.5, ('Noroeste', 'Suroeste'): 3.2, ('Noroeste', 'Sureste'): 3.5,

        # Desde NORESTE
        ('Noreste', 'Noroeste'): 1.5, ('Noreste', 'Noreste'): 1.0,
        ('Noreste', 'Occidente'): 2.0, ('Noreste', 'Centronorte'): 1.5,
        ('Noreste', 'Oriente'): 2.2, ('Noreste', 'Suroeste'): 3.5, ('Noreste', 'Sureste'): 3.0,

        # Desde OCCIDENTE
        ('Occidente', 'Noroeste'): 1.8, ('Occidente', 'Noreste'): 2.0,
        ('Occidente', 'Occidente'): 1.0, ('Occidente', 'Centronorte'): 1.2, # Bajío
        ('Occidente', 'Oriente'): 1.5, ('Occidente', 'Suroeste'): 2.5, ('Occidente', 'Sureste'): 2.8,

        # Desde CENTRONORTE (Bajío / Centro-Norte)
        ('Centronorte', 'Noroeste'): 2.0, ('Centronorte', 'Noreste'): 1.5,
        ('Centronorte', 'Occidente'): 1.2, ('Centronorte', 'Centronorte'): 1.0,
        ('Centronorte', 'Oriente'): 1.4, ('Centronorte', 'Suroeste'): 2.8, ('Centronorte', 'Sureste'): 3.0,

        # Desde ORIENTE (Centro / CDMX ampliado)
        ('Oriente', 'Noroeste'): 2.5, ('Oriente', 'Noreste'): 2.2,
        ('Oriente', 'Occidente'): 1.5, ('Oriente', 'Centronorte'): 1.4,
        ('Oriente', 'Oriente'): 1.0, ('Oriente', 'Suroeste'): 1.8, ('Oriente', 'Sureste'): 2.2,

        # Desde SUROESTE (Oaxaca, Chiapas, Guerrero)
        ('Suroeste', 'Noroeste'): 3.2, ('Suroeste', 'Noreste'): 3.5,
        ('Suroeste', 'Occidente'): 2.5, ('Suroeste', 'Centronorte'): 2.8,
        ('Suroeste', 'Oriente'): 1.8, ('Suroeste', 'Suroeste'): 1.0, ('Suroeste', 'Sureste'): 1.5,

        # Desde SURESTE (Península, Tabasco)
        ('Sureste', 'Noroeste'): 3.5, ('Sureste', 'Noreste'): 3.0,
        ('Sureste', 'Occidente'): 2.8, ('Sureste', 'Centronorte'): 3.0,
        ('Sureste', 'Oriente'): 2.2, ('Sureste', 'Suroeste'): 1.5, ('Sureste', 'Sureste'): 1.0
    }
    ...
    def _obtener_factor_distancia(region_a, region_b) -> float:
        ...
        # Busca la distancia exacta en la matriz topológica (A->B o B->A)
        dist = GestorInteracciones._DISTANCIAS.get((tipo_a, tipo_b),
                GestorInteracciones._DISTANCIAS.get((tipo_b, tipo_a), 3.0))

        # Modelo gravitacional (decaimiento por la distancia)
        return 1.0 / (dist ** 1.5)
...
class FuerzaLaboral:
    ...
    def contratar_talento(self, n_vacantes: int, pool_mercado: List['Trabajador'],
        ...
        for candidato in candidatos:
            ...
            if candidato.aceptar_oferta(oferta_salarial):
                ...
                # Guardar sector anterior ANTES de reasignar para poder comparar
                sector_anterior = candidato.sector_actual
                ...
                # Ahora la comparación es válida porque usamos el sector original
                if sector_anterior is not None and sector_anterior != self.tipo_sector:
    ...
    def _actualizar_metricas_internas(self):
        ...
        # BUG FIX: salario no siempre existe (solo tras aceptar_oferta). Usar getattr para seguridad.
        salarios = [getattr(t, 'salario', 0.0) for t in self.trabajadores]
...
class Corporacion:
    ...
    def __init__(self, id_corporacion: int, tipo_sector: str, id_region: int,
        ...
        # Evaluación robusta del estado regional: funciona con Enum y con string
    ...
    def obtener_estado_actual(self) -> Dict[str, Any]:  # BUG FIX: faltaba el parámetro self
...
class Region:
    ...
    def __init__(self, id_region: int, tipo_region: TipoRegion,
        ...
        # Etapa del ciclo económico (derivada del estado y del desarrollo geográfico)
        # Integra EtapaEconomica como dimensión complementaria a TipoRegionEconomica
        self.etapa_economica = self._determinar_etapa_economica()
    ...
    def _determinar_etapa_economica(self) -> EtapaEconomica:
        """
        Deriva la EtapaEconomica a partir del estado tipológico y del nivel
        de desarrollo geográfico de la región.
        Cruza dos dimensiones: TipoRegionEconomica (trampa) y CaracteristicasRegionales.
        """
        desarrollo = self.caracteristicas.desarrollo_economico

        if self.tipo_region_economica_actual == TipoRegionEconomica.ATRAPADA:
            # Regiones atrapadas con bajo desarrollo → subsistencia pura
            # Con algo más de desarrollo → pueden tener manufactura básica emergente
            if desarrollo < 0.35:
                return EtapaEconomica.SUBSISTENCIA
            else:
                return EtapaEconomica.INDUSTRIALIZACION

        elif self.tipo_region_economica_actual == TipoRegionEconomica.EN_TRANSICION:
            return EtapaEconomica.DIVERSIFICACION

        else:  # NO_ATRAPADA
            return EtapaEconomica.CONOCIMIENTO
    ...
    def _inicializar_estructura_economica(self):
        ...
        La selección de sectores se guía por EtapaEconomica (4 fases) para mayor
        granularidad que la simple dicotomía atrapada/no-atrapada.
        """
        # A. Selección de Sectores — guiada por EtapaEconomica
        if self.etapa_economica == EtapaEconomica.SUBSISTENCIA:
            sectores_base = ['Primario_Bajo', 'Servicios_Bajos', 'Turismo_Masivo']
            n_corps = random.randint(3, 6)
            ...
            sectores_base = ['Manufactura_Baja', 'Manufactura_Alta', 'Servicios_Intermedios',
                             'Servicios_Bajos', 'Turismo_Especializado']
            n_corps = random.randint(8, 12)
            perfil_laboral = 'PROMEDIO_NACIONAL'

        else:  # CONOCIMIENTO
            sectores_base = ['Manufactura_Alta', 'Servicios_Avanzados',
                             'Servicios_Intermedios', 'Turismo_Especializado']
    ...
    def ejecutar_paso_simulacion(self, paso_tiempo: int, contexto_global: Dict[str, float]) -> Dict[str, Any]:
        ...
        for corp in self.corporaciones:
            ...
            if fl_sector:
                fl_sector.actualizar_requisitos_tecnologicos(corp.capacidad_tecnologica)
        ...
        for corp in self.corporaciones:
            ...
            # 2.B Transición Sectorial de Markov
            matriz_cluster = self._determinar_tipo_aglomeracion()
            todas_matrices = ConfiguracionAglomeraciones.obtener_matrices_transicion()
            matriz_actual = todas_matrices.get(matriz_cluster, todas_matrices['C1'])

            # Mapeo fijo de índices a nombres de sectores según obtener_matrices_transicion()
            orden_sectores = [
                'Primario_Bajo', 'Primario_Alto',
                'Manufactura_Baja', 'Manufactura_Alta',
                'Servicios_Bajos', 'Servicios_Intermedios', 'Servicios_Avanzados',
                'Turismo_Masivo', 'Turismo_Especializado'
            ]

            if corp.tipo_sector in orden_sectores:
                idx_actual = orden_sectores.index(corp.tipo_sector)
                probs = matriz_actual[idx_actual]

                # Sorteo de nuevo sector
                nuevo_sector = random.choices(orden_sectores, weights=probs, k=1)[0]

                if nuevo_sector != corp.tipo_sector:
                    corp.tipo_sector = nuevo_sector
                    # Penalización por costo de ajuste/reorganización sectorial
                    corp.capacidad_tecnologica *= 0.95

                    if nuevo_sector not in self.sectores_activos:
                        self.sectores_activos.append(nuevo_sector)
                        # Reasignar/Crear Fuerza Laboral
                        fl = FuerzaLaboral(nuevo_sector, self.id)
                        fl.region = self
                        candidatos = [t for t in self.pool_trabajadores if t.sector_actual == nuevo_sector or t.sector_actual is None]
                        fl.contratar_talento(n_vacantes=40, pool_mercado=candidatos, presupuesto_salarial=10000)
                        self.fuerzas_laborales[nuevo_sector] = fl
        ...
        if nuevo_estado_str != estado_previo_str:
            ...
            # Actualizar también la EtapaEconomica para mantener coherencia interna
            self.etapa_economica = self._determinar_etapa_economica()
...
class ModeloEconomicoRegional:
    ...
    def ejecutar_simulacion(self, num_pasos: int = 100) -> pd.DataFrame:
        """
...
def crear_tabla_resumen_tipologico(resultados: pd.DataFrame, save_path: str):
    ...
    cols_interes = [
        ...
        'spillovers_recibidos'  # BUG FIX: nombre correcto de la columna en el DataFrame
    ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n🚀 Ejecutando simulación principal...")
        modelo_instancia, df_resultados = ejecutar_analisis_completo()

        if df_resultados.empty:
            print("❌ Error: No se generaron datos de simulación")
            exit()

        # 3. Guardar datos crudos
        print("\n💾 Guardando datos...")
        csv_path = os.path.join(save_path, 'resultados_modelo_economico.csv')
        df_resultados.to_csv(csv_path, index=False)
        print(f"   -> Datos guardados: {csv_path}")

        # 4. Generar visualizaciones básicas (sin módulos problemáticos)
        print("\n📊 Generando visualizaciones básicas...")

        # A. Gráficos principales
        try:
            crear_visualizaciones_optimizadas_por_clase(df_resultados, save_path)
            print("   ✅ Visualizaciones principales generadas")
        except Exception as e:
            print(f"   ⚠️ Error en visualizaciones principales: {e}")

        # B. Tablas de resumen
        try:
            crear_tabla_resumen_tipologico(df_resultados, save_path)
            print("   ✅ Tablas de resumen generadas")
        except Exception as e:
            print(f"   ⚠️ Error en tablas de resumen: {e}")

        # C. Análisis técnico
        try:
            analizar_evolucion_cambio_tecnico(df_resultados, save_path)
            print("   ✅ Análisis de cambio técnico completado")
        except Exception as e:
            print(f"   ⚠️ Error en análisis técnico: {e}")

        # D. Gráficos de correlación avanzados
        try:
            crear_graficos_correlacion_optimizados(df_resultados, save_path)
            print("   ✅ Gráficos de correlación avanzados generados")
        except Exception as e:
            print(f"   ⚠️ Error en gráficos de correlación: {e}")

        # E. Evaluación de procesos emergentes
        try:
            evaluar_procesos_emergentes_optimizados(df_resultados, save_path)
            print("   ✅ Evaluación de procesos emergentes completada")
        except Exception as e:
            print(f"   ⚠️ Error en procesos emergentes: {e}")

        # F. Análisis de trampa laboral
        try:
            analizar_trampa_innovacion_mercado_laboral(df_resultados, save_path)
            print("   ✅ Análisis de trampa laboral completado")
        except Exception as e:
            print(f"   ⚠️ Error en análisis laboral: {e}")

        print("\n" + "="*60)
        print("🎉 PROCESO COMPLETADO CON ADVERTENCIAS")
        print("="*60)
        print(f"📂 Resultados en: {save_path}")

        # Listar archivos generados
        archivos = os.listdir(save_path)
        print(f"\n📁 Archivos generados ({len(archivos)}):")
        for archivo in sorted(archivos):
            if archivo.endswith('.png') or archivo.endswith('.csv'):
                print(f"   - {archivo}")
