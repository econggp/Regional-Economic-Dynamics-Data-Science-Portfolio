...
Original file is located at
    https://colab.research.google.com/drive/1mkTGrxIJrQby_7NlGRFTjuvYFZghNL0_
"""
...
# Parámetros globales
INITIAL_TECH = 0.12  # Nivel tecnológico inicial
...
TECH_DEVELOPMENT_COST = 0.15  # Costo de desarrollo tecnológico (por costos burocráticos)
...
INNOVATION_BASE_RATE = 0.015  # Tasa base de innovación
MIN_INNOVATION_THRESHOLD = 0.05  # Umbral mínimo de tecnología para innovar significativamente
KNOWLEDGE_SPILLOVER = 0.18  # Efecto de derrame de conocimiento (Bajo por desconexión entre sectores)
...
LEARNING_RATE = 0.08 # Tasa de aprendizaje para mejora de habilidades
TECH_PRODUCTIVITY_IMPACT = 0.55 # Impacto de la tecnología en la productividad
WORKFORCE_PRODUCTIVITY_IMPACT = 0.6 # Impacto de las habilidades de la fuerza laboral en la productividad
SECTOR_CHANGE_BASE_RATE = 0.03 # Tasa base de cambio de sector económico para una región
...
DIVERSITY_INNOVATION_BONUS = 0.2 # Bonus a la innovación por diversidad sectorial
...
ABSORPTION_CAPACITY_FACTOR = 0.6  # Influencia de habilidades/tecnología en la capacidad de absorción de conocimiento externo
POLICY_SUPPORT_INNOVATION = 0.1  # Multiplicador base para apoyo de políticas a la innovación (puede variar en escenarios)
FDI_ATTRACTIVENESS_FACTOR = 0.3  # Influencia en spillovers basado en tecnología/margen para atraer Inversión Extranjera Directa
PRIVATE_RD_PROPENSITY = 0.09  # Propensión base a invertir en I+D privado
TRAP_ESCAPE_PROBABILITY = 0.02  # Probabilidad base de escapar de la trampa de bajo desarrollo si mejoran condiciones
SKILL_GAP_INNOVATION_PENALTY = 0.56  # Penalización a la innovación por brecha de habilidades (Ajustado por informalidad)
...
# Definir la distribución de sectores económicos
SECTOR_DISTRIBUTION = {
...
# Ajustar las métricas relacionadas con los sectores
INNOVATION_FACTOR = {
    'Agricultura Tradicional': 0.1,
    'Agricultura Tecnificada': 0.7,
    'Minería Básica': 0.3,
    'Minería Tecnológica': 0.8,
    'Manufactura Básica': 0.4,
    'Manufactura Avanzada': 0.9,
    'Servicios Básicos': 0.2,
    'Servicios Financieros': 0.6,
    'Servicios Tecnológicos': 0.95,
    'Turismo Masivo': 0.3,
    'Turismo Especializado': 0.6
}

PRODUCTIVITY_FACTOR = {
    'Agricultura Tradicional': 0.3,
    'Agricultura Tecnificada': 0.8,
    'Minería Básica': 0.5,
    'Minería Tecnológica': 0.9,
    'Manufactura Básica': 0.6,
    'Manufactura Avanzada': 0.85,
    'Servicios Básicos': 0.4,
    'Servicios Financieros': 0.7,
    'Servicios Tecnológicos': 0.9,
    'Turismo Masivo': 0.5,
    'Turismo Especializado': 0.8
}
...
class Worker:
    ...
    def __init__(self, sector_type=None):
        ...
        self.experience = 0 # Añadir atributo de experiencia inicial
    ...
    def improve_skills(self, training_investment, tech_exposure):
        ...
        # Efecto de la experiencia (aprendizaje por la práctica)
        experience_factor = 1 + np.log1p(self.experience) * 0.1 # Efecto logarítmico de la experiencia

        skill_gain = (training_effect + tech_effect) * individual_factor * experience_factor * LEARNING_RATE # Usar LEARNING_RATE global
        self.skill = np.clip(self.skill + skill_gain, 0.1, 1.0)

        if self.sector_type:
           specialized_gain = (training_effect * 1.2 + tech_effect * 0.8) * individual_factor * experience_factor * LEARNING_RATE * 1.1 # Experiencia afecta más la habilidad especializada
...
class Workforce:
    ...
    def update_metrics(self):
        ...
        # Requisitos sensibles a la tecnología
    ...
    def increment_experience(self):
        """Incrementa la experiencia de los trabajadores empleados."""
        for worker in self.workers:
            worker.experience += 1
...
class Region:
    ...
    def __init__(self, region_id, worker_pool, sector = None, innovation = None, productivity = None):
        self.id = region_id
        ...
        if sector is not None:
            ...
            for sector_id, sector_name_value in SECTORS.items():
                if sector_name_value == sector:
                    self.sector_id = sector_id
                    break
            else:

                raise ValueError(f"Sector name '{sector}' not found in SECTORS dictionary.")
        ...
        if innovation is not None:
            self.innovation_rate = innovation
    ...
    def innovate(self):
        ...
        # Transferencia tecnológica (Spillovers) - Modificado
        if hasattr(self, 'other_regions') and self.other_regions:
            # Capacidad de absorción basada en habilidades y tecnología propia
            absorption_capacity = ABSORPTION_CAPACITY_FACTOR * (self.tech_capability + self.workforce.avg_skill) / 2
            absorption_capacity = np.clip(absorption_capacity, 0.1, 1.0)

            # Atracción de FDI (simplificado, basado en margen y tech)
            fdi_attractiveness = FDI_ATTRACTIVENESS_FACTOR * (self.profit_margin + self.tech_capability) / 2

            # Spillovers potenciales de vecinos y FDI
            avg_neighbor_tech = np.mean([r.tech_capability for r in self.other_regions])
            potential_spillover_tech = max(0, avg_neighbor_tech - self.tech_capability)

            # Spillover efectivo limitado por capacidad de absorción y atractivo FDI
            effective_spillover = KNOWLEDGE_SPILLOVER * potential_spillover_tech * absorption_capacity * (1 + fdi_attractiveness / 2)
            self.tech_capability = min(1.0, self.tech_capability + effective_spillover)
            # Bonus a la tasa de innovación por spillover efectivo
            self.innovation_rate += effective_spillover * 0.1

        # Inversión en I+D privada (basado en propensión y margen)
        if random.random() < PRIVATE_RD_PROPENSITY * (self.profit_margin / self.params['margin_range'][1]):
            private_rd_investment = random.uniform(0.01, 0.05) * self.profit_margin
            self.tech_capability = min(1.0, self.tech_capability + private_rd_investment)
            self.innovation_rate += private_rd_investment * 0.2 # Pequeño bonus a la tasa

        # Penalización por brecha de habilidades
        skill_gap_penalty = SKILL_GAP_INNOVATION_PENALTY * self.workforce.qualification_gap
        self.innovation_rate *= (1 - skill_gap_penalty)

        # Apoyo de políticas (multiplicador)
        self.innovation_rate *= POLICY_SUPPORT_INNOVATION

        # Asegurar que la tasa de innovación no sea negativa
        self.innovation_rate = max(0, self.innovation_rate)

        # Evento de innovación (ocurre si se supera un umbral aleatorio)
        if random.random() < self.innovation_rate:
            # Ganancia tecnológica por innovación endógena
            tech_gain = random.uniform(0.02, 0.10) * (1 + self.diversification) * (self.tech_capability + 0.1)
            self.tech_capability = min(1.0, self.tech_capability + tech_gain)

            # Posibilidad de introducir nuevo producto debido a la innovación
            if random.random() < 0.3:
                # Crear producto ligeramente más avanzado o diversificado
                if random.random() < 0.7: # Incremental o relacionado
                    new_product = Product(self.sector_type, len(self.products) + 1)
                    new_product.tech_intensity = min(1.0, self.tech_capability * random.uniform(0.8, 1.1))
                else: # Más radical
                    random_sector = random.choice(list(SECTOR_PARAMS.keys()))
                    new_product = Product(random_sector, len(self.products) + 1)
                self.products.append(new_product)
                self.total_production += new_product.production
        # else: # Si no hay evento de innovación, no resetear la tasa a 0
        #     self.innovation_rate = max(0, self.innovation_rate * 0.9) # Decaimiento leve si no hay evento
    ...
    def update_workforce(self):
        ...
        # Incrementar experiencia de los trabajadores empleados
        self.workforce.increment_experience()
    ...
    def check_innovation_trap(self):
        """Determina si la región cae o escapa de la trampa de innovación"""
        # Condiciones para estar atrapado (más estrictas)
        trap_conditions = (
            self.tech_capability < 0.35, # Umbral ligeramente más alto
            self.innovation_rate < 0.04, # Tasa de innovación persistentemente baja
            self.workforce.qualification_gap > 0.35, # Brecha de habilidades alta
            self.diversification < 0.25, # Baja diversificación
            len(self.products) <= 4 # Pocos productos
        )
        is_currently_trapped = sum(trap_conditions) >= 3 # Necesita cumplir al menos 3 condiciones

        if not self.trapped and is_currently_trapped:
            self.trapped = True
            # Efectos al caer en la trampa (más sutiles)
            self.profit_margin = min(0.9, self.profit_margin * 1.01) # Ligero aumento de margen (complacencia)
            self.tech_capability = max(0.05, self.tech_capability * 0.99) # Ligero deterioro tecnológico
            self.workforce.training_investment *= 0.9 # Reducción inversión en formación

        elif self.trapped and not is_currently_trapped:
            # Condiciones para escapar de la trampa (requiere mejora significativa)
            escape_conditions = (
                self.tech_capability > 0.4,
                self.innovation_rate > 0.06,
                self.workforce.qualification_gap < 0.25,
                self.diversification > 0.3
            )
            can_escape = sum(escape_conditions) >= 3

            if can_escape and random.random() < TRAP_ESCAPE_PROBABILITY * (1 + POLICY_SUPPORT_INNOVATION):
                self.trapped = False
                # Bonus al escapar (recuperación)
                self.tech_capability *= 1.05
                self.innovation_rate *= 1.2
                self.workforce.training_investment = self.params['training_investment'] # Restaurar inversión

        # Efectos continuos si sigue atrapado
        if self.trapped:
            self.productivity *= 0.99 # Ligera penalización continua a la productividad
            self.innovation_rate *= 0.95 # Dificultad para innovar
...
class EconomicModel:
    ...
    def __init__(self, n_regions=32):
        ...
        for i in range(n_regions):
            sector = np.random.choice(list(normalized_dist.keys()),
                                    p=list(normalized_dist.values()))
            innovation = np.random.normal(INNOVATION_FACTOR[sector], 0.1)
            productivity = np.random.normal(PRODUCTIVITY_FACTOR[sector], 0.1)
            self.regions.append(Region(
            region_id=i,
            worker_pool=self.worker_pool,
            sector=sector,
            innovation=np.clip(innovation, 0, 1),
